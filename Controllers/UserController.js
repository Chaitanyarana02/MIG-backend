...
async function sendclaimWithPassingQuestionImages(req, res) {
  try {
    const {f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13, QUITSIMAGES } = req.body;
    ...
    const allImagesWherePrevioulyImagesWasAdded = await QuitsImages.findAll({where: { sendClaimId: createClaim.id }});
    const rearrangedImages = allImagesWherePrevioulyImagesWasAdded.map(image => ({
                                                                                        f1: image.f1,
                                                                                        f2: image.f2,
                                                                                        f3: image.f3
                                                                                    }));
    ...
    createClaim.quitsNo = response.data.quitsNo;
    const finalSendClaim = await SendClaim.update(
      { quitsNo: createClaim.quitsNo },
      { where: { id: createClaim.id } } 
    );
    res.json({
      status: 'success',
      message: 'Data sent successfully',
      data: responseData,
      apiRespons:response.data,
      allImagesWherePrevioulyImagesWasAdded:rearrangedImages,
      quitsNo: quitsNo,
      RegisterNo:RegisterNo,
      sendClaim:finalSendClaim,
      processedFiles: processedFiles
    });
  } catch (error) {
    console.error('Error occurred while calling external API:', error);
...
async function updateclaimWithPassingQuestionImages(req, res) {
    try {
    const {f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13, QUITSIMAGES } = req.body;
    ...
    if (customer) {
     ...
     for (let i = 0; i < QUITSIMAGES.length; i++) {
      ...
      const existingRecord = await QuitsImages.findOne({
        where: { sendClaimId: f1 }
      });

      console.log("hello",existingRecord);
...
module.exports = {
  ...
  quitsdelete,
  sendclaim,
  sendclaimWithPassingQuestionImages,
  updateclaim,
  updateclaimWithPassingQuestionImages
